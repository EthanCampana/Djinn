...
func TestDefineMacros(t *testing.T) {
	input := `
	cr number = 1;
	cr function = fn(x, y) { x + y };
	cr mymacro = macro(x, y) { x + y; };
`
...
func TestExpandMacros(t *testing.T) {
	tests := []struct {
		...
		{
			`
cr infixExpression = macro() { quote(1 + 2); };
infixExpression();
`,
			`(1 + 2)`,
		},
		{
			`
cr reverse = macro(a, b) { quote(unquote(b) - unquote(a)); };
reverse(2 + 2, 10 - 5);
...
`,
		...
		},
		{
			...
			`
cr unless = macro(condition, consequence, alternative) {
